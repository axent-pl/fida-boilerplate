...
app.add_middleware(SessionMiddleware,
                   secret_key=config.settings.CS_SESSION_SECRET)
...
async def get_authorization_grants(request: Request, client_id: str, scope: str, reponse_type: str, redirect_uri: str, consent_service: ConsentService = Depends(ConsentService), user: dto.UserDTO = Depends(components.get_user), client_service: ClientService = Depends(ClientService)):
    consents = consent_service.find_all_by_username_and_client_id(
        username=user.username, client_id=client_id)
...
async def save_authorization_grants(request: Request, client_id: str, scope: str, reponse_type: str, redirect_uri: str, consent_service: ConsentService = Depends(ConsentService), user: dto.UserDTO = Depends(components.get_user), oauth_service: OAuthService = Depends(OAuthService), client_service: ClientService = Depends(ClientService)):
    consents = consent_service.find_all_by_username_and_client_id(
        username=user.username, client_id=client_id)
    ...
    authorization_code = oauth_service.get_authorization_code(
        redirect_uri=redirect_uri, client_id=client_id, username=user.username)
...
async def issue_token(
        ...
        grant_type: str = Form(),
        consent_service: ConsentService = Depends(ConsentService),
    ...
    if grant_type == 'authorization_code':
        authorization_request = oauth_service.get_authorization_request(
            authorization_code=code, redirect_uri=redirect_uri, client_id=client_id)
        if not authorization_request:
            raise HTTPException(
                status_code=400, detail="Invalid authorization_code")
        consents = consent_service.find_all_by_username_and_client_id(
            username=authorization_request.get('username'), client_id=client_id)
        claims = {
        ...
        token = oauth_service.get_token(
            key=oauth_service.get_key(self_generated=True),
            issuer_uri=request.client.host,
            username=authorization_request.get('username'),
            client_id=client_id,
            claims=claims
        )
        return token
